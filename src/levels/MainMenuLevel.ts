...
export class MainMenuLevel implements Level {
  ...
  private setupMenu(): void {
    ...
    this.menuContainer.className = "game-main-menu";
    ...
    // Create menu buttons with entities
  ...
  private handleSelectionChanged(data: {
    ...
    for (const entity of this.menuEntities) {
      const button = (entity as any).domElement as HTMLButtonElement;
   ...
   */
  public start(): void {
    this.logger.info("Main menu level started");

    // Setup UI (DOM entity is now persistent, no need to set it up here)
    this.setupMenu();

    // Start animation loop for world updates
   ...
   */
  public stop(): void {
    this.logger.info("Main menu level stopped");
    if (this.animationFrameId !== null) {
  ...
  /**
   * Animation loop for the main menu
   */
  private animate(): void {
    ...
    // Update the world (which will update all systems)
   ...
   */
  public cleanup(): void {
    this.logger.info("Cleaning up main menu level");
    ...
    this.logger.debug("Main menu cleanup complete");
  }
}
